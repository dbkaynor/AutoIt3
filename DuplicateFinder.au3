...
#cs
    ...
    Highlite items if size is identical
    Hide items that are not identical size
...
Func SplitRawData()
    ...
    For $A In $FilteredArray
        ...
        If IsArray($ft) Then
            ...
            $ts = $B[$B[0]] & "~" & $A & "~" & $C[1] & "~" & $C[2] & "~" & $fs & "~" & $ft1 & "~" & 'crc' & "~" & FileGetAttrib($A)
...
Func SingleRecycle()
    ...
    Local $R = MsgBox(4, "Recycle single", "Are you sure that you want to recycle this item?" & @CRLF & "(Move to recycle bin.)")
    If $R = 6 Then ; yes
...
Func SingleDelete()
    ...
    Local $R = MsgBox(4, "Delete single", "Are you sure that you want to delete this item?" & @CRLF & "(This is permanent!)")
    If $R = 6 Then ; yes
        $R = GUICtrlRead($InputSingle)
        If FileExists($R) Then
