...
Global $MainForm = GUICreate($ProgramName & "  " & $FileVersion, 760, 600, 20, 20, $MainFormOptions)
...
GUICtrlCreateGroup("Loop", 425, 90, 265, 45)
...
;"text", left, top [, width [, height
GUICtrlCreateGroup("Filter event type", 15, 190, 220, 45)
Global $InputFilterType = GUICtrlCreateInput("", 25, 205, 130, 25)
GUICtrlSetTip($InputFilterType, "Input a filter string. (Supports regular expressions)")
Global $CheckExcludeType = GUICtrlCreateCheckbox("Exclude", 170, 205, 60, 25)
GUICtrlSetTip($CheckExcludeType, "Exclude or include the filter string")
GUICtrlCreateGroup("", -99, -99, 1, 1)

GUICtrlCreateGroup("Filter event source", 250, 190, 220, 45)
Global $InputFilterSource = GUICtrlCreateInput("", 260, 205, 130, 25)
GUICtrlSetTip($InputFilterSource, "Input a filter string. (Supports regular expressions)")
Global $CheckExcludeSource = GUICtrlCreateCheckbox("Exclude", 400, 205, 60, 25)
GUICtrlSetTip($CheckExcludeSource, "Exclude or include the filter string")
GUICtrlCreateGroup("", -99, -99, 1, 1)

GUICtrlCreateGroup("Find", 490, 190, 220, 45)
Global $InputSearchString = GUICtrlCreateInput("", 505, 205, 130, 25)
GUICtrlSetTip($InputSearchString, "Input a data filter. Supports regular expressions")
Global $ButtonSearch = GUICtrlCreateButton("Find", 640, 205, 51, 25)
GUICtrlSetTip($ButtonSearch, "Exclude or include the filter string")
GUICtrlCreateGroup("", -99, -99, 1, 1)

Global $LabelStatus = GUICtrlCreateLabel("Status main", 20, 250, 700, 25, $SS_SUNKEN)
GUICtrlSetTip($LabelStatus, "Status message")
Global $ListView = GUICtrlCreateListView("Number|Event type|Date logged|Event source|Event description", 20, 290, 700, 280, -1, BitOR($WS_EX_CLIENTEDGE, $LVS_EX_GRIDLINES, $LVS_EX_FULLROWSELECT))
GUICtrlSendMsg($ListView, $LVM_SETCOLUMNWIDTH, 0, 50)
GUICtrlSendMsg($ListView, $LVM_SETCOLUMNWIDTH, 1, 50)
GUICtrlSendMsg($ListView, $LVM_SETCOLUMNWIDTH, 2, 50)
...
Func GetStats($hEventLog)
    ...
    For $x = 1 To _EventLog__Count($hEventLog)
        ...
        If StringLen($TmpString) > 0 Then ; There is a tmpstring
            ...
            If StringLen(GUICtrlRead($InputFilterType)) = 0 And StringLen(GUICtrlRead($InputFilterSource)) = 0 Then ; There are no filters
            ...
            Else ;Here we have at least one filter
                ...
                If UBound($F) > 1 Then
                    ...
                    $result = StringRegExp($F[1], GUICtrlRead($InputFilterType), 0) ; EVENT TYPE filter. Returns 1 (matched) or 0 (no match)
                    If $result = 1 Then DisplayResults($E, $Counter)

                    $result = StringRegExp($F[1], GUICtrlRead($InputFilterSource), 0) ; EVENT SOURCE filter. Returns 1 (matched) or 0 (no match)
                    If $result = 1 Then DisplayResults($E, $Counter)
...
Func SetDefaults($Type = 'all')
    ...
    WinMove($ProgramName, "", 1000, 10, 760, 600)
    ...
    GUICtrlSetState($CheckExcludeSource, $GUI_UNCHECKED)
    GUICtrlSetState($CheckExcludeType, $GUI_UNCHECKED)
    GUICtrlSetState($CheckLoop, $GUI_UNCHECKED)
    GUICtrlSetData($InputFilterType, "")
...
Func SaveProject()
    ...
    FileWriteLine($file, "CheckExcludeType:" & GUICtrlRead($CheckExcludeType))
    FileWriteLine($file, "CheckExcludeSource:" & GUICtrlRead($CheckExcludeSource))
    ...
    FileWriteLine($file, "InputFilterType:" & GUICtrlRead($InputFilterType))
...
Func LoadProject($Type)
    ...
    While 1
        ...
        If StringInStr($LineIn, "CheckExcludeType:") Then GUICtrlSetState($CheckExcludeType, StringMid($LineIn, StringInStr($LineIn, ":") + 1))
        If StringInStr($LineIn, "CheckExcludeSource:") Then GUICtrlSetState($CheckExcludeSource, StringMid($LineIn, StringInStr($LineIn, ":") + 1))
        ...
        If StringInStr($LineIn, "InputFilterType:") Then GUICtrlSetData($InputFilterType, StringMid($LineIn, StringInStr($LineIn, ":") + 1))
