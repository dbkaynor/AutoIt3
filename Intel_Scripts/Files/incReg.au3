...
;** 
;** Update for Rev 2.1.1		- Jarek Szymanski 2/17/2009
;**		- Added Support for Windows 7 in InstallSRIGDAlready() function (graphic install check)
;** 
;** Update for Rev 2.1.2		- Jarek Szymanski 3/24/2009
;**		- Added support for AUB graphic driver install reg key in InstallSRIGDAlready() function (graphic install check)
;** 
;** Update for Rev 2.1.3		- Jarek Szymanski 8/13/2009
;**		- Added support for SandyBridge
;**
;** Update for Rev 2.1.4		- Jarek Szymanski 1/14/2009
;**		- Added support for Cougar Point family - Huron River CRB
;** 
;******************************************************************************************
#comments-end HEADER
...
Func InstallSRIGDAlready()
	...
	If (IsWinXP64Family() Or IsWinVista64() Or IsWin7_64()) Then
		$strKey = "HKLM64\Software\Intel\IGDI"
		$strKey1 = "HKLM\Software\Wow6432Node\Intel\GFX" ;newest AUB driver changed registry key
	Else
		$strKey = "HKLM\Software\Intel\IGDI"	
		$strKey1 = "HKLM\Software\Intel\GFX"	;newest AUB driver changed registry key
	EndIf
	$strSubKey = "Install"
		
	$var = RegRead($strKey, $strSubKey)  
	$var = StringUpper($var) 
	;newest AUB driver changed registry key
	$var1 = RegRead($strKey1, $strSubKey)  
	$var1 = StringUpper($var1)
	
	;If ($var Or $var64 Or $var32) == "SUCCESS" Then
	If $var == "SUCCESS" Or $var1 == "SUCCESS" Then
		Return True
	Else
		Return False
	EndIf
...
Func GetRSDT(ByRef $strFamily, ByRef $strPlatform)
	...
	Switch StringUpper($strKey)
		...
		; Added for IBEXPEAK Calpella CRB	
		Case "CALPELAC"
			$strFamily = "IbexPeak"
			$strPlatform = "Calpella (IbexPeak)"
		; Added for IBEXPEAK Calpella CRB	
		Case "SNDYBRDG"
			$strFamily = "SandyBridge"
			$strPlatform = "HuronRiver (SandyBridge / IbexPeak)"	
		; Cougar Point family - Huron River CRB	
		Case "SNB-CPT"
			$strFamily = "SandyBridge-CPT"
			$strPlatform = "HuronRiver (SandyBridge / Cougar Point)"			
		; GENERIC
		Case Else
			$strFamily = "GENERIC"
			$strPlatform = "GENERIC"
