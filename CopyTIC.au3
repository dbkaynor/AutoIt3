...
GUICtrlSetResizing(-1, $GUI_DOCKALL)
Global $ButtonNetcopy = GUICtrlCreateButton("Copy", 150, 10, 70, 25)
GUICtrlSetResizing(-1, $GUI_DOCKALL)
...
Func LoadProject($type)
	...
	If $file = -1 Then
		_Debug(@ScriptLineNumber & " Unable to open file for reading: " & $Projectfilename, '', '', True)
		Return
	EndIf
	...
	If StringInStr(FileReadLine($file, 1), "Project file for " & $ProgramName) <> 1 Then
		...
		_Debug(@ScriptLineNumber & " Not a Project file for " & $ProgramName, '', '', True)
		Return
	EndIf
	...
	If Not FileExists(GUICtrlRead($ComboSource)) Then
		_Debug(@ScriptLineNumber & " TIC source directory not found " & @CRLF & GUICtrlRead($ComboSource), '', '', True)
	EndIf
	If Not FileExists(GUICtrlRead($ComboDestination)) Then
		_Debug(@ScriptLineNumber & " TIC destination directory not found " & GUICtrlRead($ComboDestination), '', '', True)
	EndIf
...
Func SaveProject()
	...
	If $file = -1 Then
		_Debug(@ScriptLineNumber & " Unable to open file for writing: " & $Projectfilename, '', '', True)
		Exit
	EndIf
...
Func GetTICList()
	...
	_GUICtrlListBox_ResetContent($ListTICInfo)
	If Not FileExists(GUICtrlRead($ComboSource)) Then
		_Debug(@ScriptLineNumber & " TIC source directory not found " & GUICtrlRead($ComboSource), '', '', True)
		Return
	EndIf
	...
	If $search = -1 Then
		_Debug(@ScriptLineNumber & " No matches found: " & $FileSearchString, '', '', True)
		Return
	EndIf
...
Func GetTICInfo()
	_Debug(@ScriptLineNumber & " GetTICInfo")
	_GUICtrlListBox_ResetContent($ListTICInfo)
	...
	If Not FileExists($TmpS) Then
		_debug(@ScriptLineNumber & ' GetTicInfo.   Invalid folder: ' & @CRLF & $TmpS, '', '', True)
		Return
	EndIf
...
Func CopyTIC()
	...
	If StringLen(GUICtrlRead($InputTICName)) < 3 Then ; verify that the TIC name is ok
		_Debug(@ScriptLineNumber & " TIC name is not valid: " & GUICtrlRead($InputTICName), '', '', True)
		Return
	EndIf
